...
<div class="navbar-custom-menu">
    ...
    <ol class="breadcrumb">
      ...
      <li><a href="{{url('')}}/admin/products" class="text-capitalize">products</a></li>
 ...
 <div class="row">
         <!-- THE ACTUAL CONTENT -->
         <input type="text" id="EditIdBox" name="" style="display: none">
         <div class="col-md-12">
           <div class="box">
             <div class="box-header with-border">
               <a href="{{url('admin/products/create')}}" class="btn btn-primary ladda-button" data-style="zoom-in"><span class="ladda-label"><i class="fa fa-plus"></i> Add product</span></a>
                 ...
                 <thead>
                   ...
                   <tbody>
                     ...
                     <tr role="row" class="odd">
                       ...
                       <td>{{$viewproduct->categorie_name}}</td>
                       <td>{{$viewproduct->sku}}</td>
                       <td>{{$viewproduct->price}}</td>
                       <td>{{$viewproduct->stock}}</td>
                       <td>{{($viewproduct->active ==1)? 'Active':'Inactive'}}</td>
                       <td>
                          <label class="switch">
                              <input type="checkbox" {{($viewproduct->offer == 1) ? 'Checked' : ''}} 
                              id="offer_id_{{$viewproduct->product_id}}"

                              onclick="offer({{$viewproduct->product_id}})"
                              >
                       ...
                       <td>
                          <label class="switch" >
                              <input type="checkbox" {{($viewproduct->premium == 1)? 'Checked' : ''}}
                              id="premium_id_{{$viewproduct->product_id}}"

                               onclick="premium({{$viewproduct->product_id}})"
                       ...
                       <td>
                          <label class="switch">
                              <input type="checkbox"
                              {{($viewproduct->delux == 1)? 'Checked' : ''}}

                              id="delux_id_{{$viewproduct->product_id}}"

                              onclick="delux({{$viewproduct->product_id}})"
                              >
                              <span class="slider"></span>
                          </label>
                      </td>
                      <td>
                          <label class="switch">
                              <input type="checkbox"
                              {{($viewproduct->collection == 1)? 'Checked' : ''}}

                              id="collection_id_{{$viewproduct->product_id}}"

                              onclick="collection({{$viewproduct->product_id}})"
                              >
                              >
                              <span class="slider"></span>
                          </label>
                     ...
                     <a href="{{url('admin/products/'.$viewproduct->product_id).'/edit'}}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> Edit</a>

                     <a onclick="delete_button({{$viewproduct->product_id}})" class="btn btn-xs btn-default"><i class="fa fa-trash"></i> Delete</a>

                      <a onclick="vendor_product({{$viewproduct->product_id}})" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> Edit Vendor Product</a>


<?php  $special=\App\Models\SpecificPrice::where(['product_id' =>$viewproduct->product_id])->count() 
?>
                      @if($special != 0)
 
                        <a onclick="Special_price_button({{$viewproduct->product_id}})" class="btn btn-xs btn-default"><i class="fa fa-trash"></i> Delete Special Price</a>
 
                      @endif


                     {{-- <a href="#" class="btn btn-xs btn-default clone-btn" data-product="{{$viewproduct->product_id}}"><i class="fa fa-clone"></i> Clone</a>     </td></tr> --}}
                     @endforeach
                   </tbody>
                   <tfoot>
                     <tr>
       ...
       <div class="modal-dialog" role="document">
         <div class="modal-content">
           ...
           <form action="{{url('admin/products/clone')}}" method="POST">
    ...
    function Special_price_button($id){
        ...
        swal({
          ...
          setTimeout(function () {
             ...
             var delete_id = $id
              $.get('{{url('admin/deletespecialprice')}}',
                 {
                     _token: "{{ csrf_token() }}",
                     deleteid:delete_id
                 }
             ...
             function(data){
                   ...
                   toastr.success('SpecailPrice Delete successfully');
...
<div id="addvendorpro" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      <form class="row contact_form" action="{{url('admin/Addvendor')}}" method="post" id="contactForm" enctype="multipart/form-data">
      <div class="modal-body">
            ...
            <div class="row" id="row_add_1">
             ...
             <div class="form-group col-md-3" style="margin-left: -10px">
                <select name="productid[]" class="form-control select2_field">
                   @foreach($product as $viewproduct)
                   <option value="{{$viewproduct->product_id}}">{{$viewproduct->name}}</option>
                   @endforeach
                </select>
             </div>
             <div class="form-group col-md-4">
              <select class="form-control" name="color[]">
                ...
                @foreach($size as $viewsize)
                  <option value="{{$viewsize->value}}">{{$viewsize->value}}</option>
                @endforeach
...
<script type="text/javascript">
  ...
  $('#Editvendor').click(function(event) {
    ...
    html += '<select style="cursor: no-drop;" name="productid[]" class="form-control">';
    html += '@foreach($product as $viewproduct)';
    
    if(editid == {{$viewproduct->product_id}}){
      
      html += '<option  value="{{$viewproduct->product_id}}">{{$viewproduct->name}}</option>';
    ...
    html += '<select class="form-control" name="color[]">@foreach($size as $viewsize)<option value="{{$viewsize->value}}">{{$viewsize->value}}</option>@endforeach</select>';
    html += '</div>';
  ...
  @if(Session::has('vendor'))
    @foreach(session('swalmess') as  $details)
      @if($details["id"] == 1 )
         toastr.success('{{$details["name"]}}');
       @else
         toastr.error('{{$details["name"]}}');
       @endif
    @endforeach
  {{session()->put('swalmess','')}}
